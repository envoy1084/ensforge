...
```ts
import type { GetResolverDelegateApprovalParameters, UseEnsQueryParameters } from "@ensforge/react";
```
...
const atom = getResolverDelegateApprovalAtom(ens, parameters, options);
```

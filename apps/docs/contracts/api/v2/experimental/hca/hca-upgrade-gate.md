...
```ts
import { hcaUpgradeGateV2Abi } from "@ensforge/contracts/v2/experimental/hca";
```
...
| Export                | Description                                             |
| --------------------- | ------------------------------------------------------- |
| `hcaUpgradeGateV2Abi` | Immutable ABI value with viem-compatible literal types. |
...
```ts
type Export = typeof hcaUpgradeGateV2Abi;
```

...
Get the lifecycle status of an ENS name.
...
const program = getNameStatus.effect(config, parameters);
...
```ts
const request = getNameStatus.request(parameters);
```
...
- [`ens.name.getNameStatus`](/sdk/api/name/get-name-status)

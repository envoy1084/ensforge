...
Get the active ENS protocol route for a name.
...
```ts
type GetProtocolResult = Awaited<ReturnType<typeof getProtocol>>;
```
...
const program = getProtocol.effect(config, parameters);
...
```ts
const request = getProtocol.request(parameters);
```
...
- [`ens.name.getProtocol`](/sdk/api/name/get-protocol)

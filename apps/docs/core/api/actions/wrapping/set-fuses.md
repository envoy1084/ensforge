...
```ts
import type { SetFusesParameters } from "@ensforge/core";
```
...
const program = setFuses.effect(config, parameters);
...
```ts
const call = setFuses.call(parameters);
```

## Error

```ts
import type { SetFusesError } from "@ensforge/core";
```
...
- [`ens.wrapping.setFuses`](/sdk/api/wrapping/set-fuses)

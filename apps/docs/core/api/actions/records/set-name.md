---
title: setName
description: Sets name for ENS resolver records.
---

# setName

Sets name for ENS resolver records.

This action belongs to ENS resolver records. It selects the supported contract and protocol route from the current configuration and name state.

## Import

```ts
import { setName } from "@ensforge/core";
```

## Usage

```ts
import { setName } from "@ensforge/core";
import { config } from "./config";

const result = await setName(config, {
  name: "example.eth",
  value: "https://example.com",
});
```

## Parameters

```ts
type SetNameParameters = Parameters<typeof setName>[1];
```

### name

`string`

ENS name used by the operation. It is normalized before contract interaction.

### value

`string`

Value written by the operation.

## Return Type

```ts
type SetNameResult = Awaited<ReturnType<typeof setName>>;
```

`CallExecutionResult`

## Effect

```ts
const effect = setName.effect(config, parameters);

type Success = Effect.Effect.Success<typeof effect>;
type Failure = Effect.Effect.Error<typeof effect>;
```

## Call

Use `.call` to prepare a write intent without submitting it.

```ts
const intent = setName.call(parameters);
```

## Error

```ts
import type { Effect } from "effect";

type SetNameError = Effect.Effect.Error<ReturnType<typeof setName.effect>>;
```

See [Error Handling](/core/guides/error-handling) for tagged errors and stable error codes.

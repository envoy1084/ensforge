---
title: getContentHash
description: Gets content hash for ENS resolver records.
---

# getContentHash

Gets content hash for ENS resolver records.

This action belongs to ENS resolver records. It selects the supported contract and protocol route from the current configuration and name state.

## Import

```ts
import { getContentHash } from "@ensforge/core";
```

## Usage

```ts
import { getContentHash } from "@ensforge/core";
import { config } from "./config";

const result = await getContentHash(config, {
  name: "example.eth",
});
```

## Parameters

```ts
type GetContentHashParameters = Parameters<typeof getContentHash>[1];
```

### name

`string`

ENS name used by the operation. It is normalized before contract interaction.

### blockNumber

`bigint | undefined`

Block number to read from. Cannot be combined with `blockTag`.

### blockTag

`"latest" | "earliest" | "pending" | "safe" | "finalized" | undefined`

Block tag to read from. Cannot be combined with `blockNumber`.

## Return Type

```ts
type GetContentHashResult = Awaited<ReturnType<typeof getContentHash>>;
```

The return type is inferred from the action and preserves its discriminated protocol and workflow states.

## Effect

```ts
const effect = getContentHash.effect(config, parameters);

type Success = Effect.Effect.Success<typeof effect>;
type Failure = Effect.Effect.Error<typeof effect>;
```

## Request

Use `.request` to include the read in [`readBatch`](/core/guides/batching).

```ts
const request = getContentHash.request(parameters);
```

## Error

```ts
import type { Effect } from "effect";

type GetContentHashError = Effect.Effect.Error<ReturnType<typeof getContentHash.effect>>;
```

See [Error Handling](/core/guides/error-handling) for tagged errors and stable error codes.

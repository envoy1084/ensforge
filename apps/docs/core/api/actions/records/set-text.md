...
```ts
import type { SetTextParameters } from "@ensforge/core";
```
...
const program = setText.effect(config, parameters);
...
```ts
const call = setText.call(parameters);
```

## Error

```ts
import type { SetTextError } from "@ensforge/core";
```
...
- [`ens.records.setText`](/sdk/api/records/set-text)

...
```ts
import type { MigrateNameParameters } from "@ensforge/core";
```
...
```ts
import type { MigrateNameResult } from "@ensforge/core";
```
...
const program = migrateName.effect(config, parameters);

---
title: getResolver
description: Gets resolver for resolver discovery and Universal Resolver calls.
---

# getResolver

Gets resolver for resolver discovery and Universal Resolver calls.
...
const result = await getResolver(config, {
  name: "example.eth",
});
```
...
```ts
import type { GetResolverParameters } from "@ensforge/core";
```
...
```ts
import type { GetResolverResult } from "@ensforge/core";
```
...
const program = getResolver.effect(config, parameters);
...
```ts
const request = getResolver.request(parameters);
```

## Error

```ts
import type { GetResolverError } from "@ensforge/core";
```
...
- [`ens.resolution.getResolver`](/sdk/api/resolution/get-resolver)

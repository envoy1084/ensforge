...
```ts
type GetAliasParameters = Parameters<typeof getAlias>[1];
```
...
```ts
const effect = getAlias.effect(config, parameters);
...
type GetAliasError = Effect.Effect.Error<ReturnType<typeof getAlias.effect>>;
```

...
const result = await ens.capabilities.getResolverRoles({
  name: "example.eth",
  account: {},
});
```
...
const program = ens.capabilities.getResolverRoles.effect(parameters);
...
```ts
const request = ens.capabilities.getResolverRoles.request(parameters);
```

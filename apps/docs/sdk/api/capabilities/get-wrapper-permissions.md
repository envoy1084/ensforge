...
const result = await ens.capabilities.getWrapperPermissions({
  name: "example.eth",
  account: {},
});
```
...
const program = ens.capabilities.getWrapperPermissions.effect(parameters);
...
```ts
const request = ens.capabilities.getWrapperPermissions.request(parameters);
```

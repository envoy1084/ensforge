...
const result = await ens.capabilities.hasRegistryRoles({
  name: "example.eth",
  account: {},
  roles: 1n,
});
...
const program = ens.capabilities.hasRegistryRoles.effect(parameters);
...
```ts
const request = ens.capabilities.hasRegistryRoles.request(parameters);
```

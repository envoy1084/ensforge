...
const result = await ens.migration.getMigrationEligibility({
  name: "example.eth",
  account: {},
});
```
...
const program = ens.migration.getMigrationEligibility.effect(parameters);
...
```ts
const request = ens.migration.getMigrationEligibility.request(parameters);
```

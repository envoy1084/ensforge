...
const result = await ens.migration.approveMigration({
  name: "example.eth",
});
```
...
const program = ens.migration.approveMigration.effect(parameters);
...
```ts
const call = ens.migration.approveMigration.call(parameters);
```

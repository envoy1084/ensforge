...
const result = await ens.permissions.revokeRegistryRoles({
  name: "example.eth",
  account: "value",
  roles: 1n,
});
...
const program = ens.permissions.revokeRegistryRoles.effect(parameters);
...
```ts
const call = ens.permissions.revokeRegistryRoles.call(parameters);
```

...
const result = await ens.permissions.approveName({
  name: "example.eth",
  approved: true,
});
```
...
const program = ens.permissions.approveName.effect(parameters);
...
```ts
const call = ens.permissions.approveName.call(parameters);
```

...
const result = await ens.permissions.revokeResolverRootRoles({
  name: "example.eth",
  account: "value",
  roles: 1n,
});
...
const program = ens.permissions.revokeResolverRootRoles.effect(parameters);
...
```ts
const call = ens.permissions.revokeResolverRootRoles.call(parameters);
```

...
const result = await ens.permissions.revokeResolverRoles({
  name: "example.eth",
  account: "value",
  record: {},
});
...
const program = ens.permissions.revokeResolverRoles.effect(parameters);
...
```ts
const call = ens.permissions.revokeResolverRoles.call(parameters);
```

## Error

```ts
import type { RevokeResolverRolesError } from "@ensforge/sdk";
```

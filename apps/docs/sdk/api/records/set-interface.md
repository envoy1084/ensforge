...
const result = await ens.records.setInterface({
  name: "example.eth",
...
const program = ens.records.setInterface.effect(parameters);
...
```ts
const call = ens.records.setInterface.call(parameters);
```

...
const result = await ens.records.setData({
  name: "example.eth",
  key: "url",
...
```ts
import type { SetDataParameters } from "@ensforge/sdk";
```
...
const program = ens.records.setData.effect(parameters);
...
```ts
const call = ens.records.setData.call(parameters);
```

## Error

```ts
import type { SetDataError } from "@ensforge/sdk";
```

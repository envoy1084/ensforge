...
```ts
type GetAvatarParameters = Parameters<typeof sdk.records.getAvatar>[0];
```
...
```ts
type GetAvatarResult = Awaited<ReturnType<typeof sdk.records.getAvatar>>;
```
...
```ts
const effect = sdk.records.getAvatar.effect(parameters);

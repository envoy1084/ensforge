...
const result = await ens.records.setContentHash({
  name: "example.eth",
  protocol: "ipfs",
...
const program = ens.records.setContentHash.effect(parameters);
...
```ts
const call = ens.records.setContentHash.call(parameters);
```

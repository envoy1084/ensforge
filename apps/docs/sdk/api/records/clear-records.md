...
const result = await ens.records.clearRecords({
  name: "example.eth",
});
```
...
const program = ens.records.clearRecords.effect(parameters);
...
```ts
const call = ens.records.clearRecords.call(parameters);
```

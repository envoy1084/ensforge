...
const result = await ens.records.getContentHash({
  name: "example.eth",
});
```
...
const program = ens.records.getContentHash.effect(parameters);
...
```ts
const request = ens.records.getContentHash.request(parameters);
```

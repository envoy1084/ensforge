...
```ts
type GetTextParameters = Parameters<typeof sdk.records.getText>[0];
```
...
```ts
type GetTextResult = Awaited<ReturnType<typeof sdk.records.getText>>;
```
...
```ts
const effect = sdk.records.getText.effect(parameters);

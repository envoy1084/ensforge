...
```ts
import type { SetTextParameters } from "@ensforge/sdk";
```
...
const program = sdk.records.setText.effect(parameters);
...
```ts
const call = sdk.records.setText.call(parameters);
```

## Error

```ts
import type { SetTextError } from "@ensforge/sdk";
```

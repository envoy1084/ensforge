...
const result = await ens.records.setAddresses({
  name: "example.eth",
  addresses: [],
});
```
...
const program = ens.records.setAddresses.effect(parameters);
...
```ts
const call = ens.records.setAddresses.call(parameters);
```

...
const result = await ens.registration.getRegistrationPlan({
  name: "example.eth",
...
const program = ens.registration.getRegistrationPlan.effect(parameters);
...
```ts
const request = ens.registration.getRegistrationPlan.request(parameters);
```

...
const result = await ens.registration.commitName({
...
const program = ens.registration.commitName.effect(parameters);
...
```ts
const call = ens.registration.commitName.call(parameters);
```

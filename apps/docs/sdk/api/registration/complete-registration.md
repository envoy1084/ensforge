...
const result = await ens.registration.completeRegistration({
  name: "example.eth",
...
const program = ens.registration.completeRegistration.effect(parameters);
...
```ts
const call = ens.registration.completeRegistration.call(parameters);
```

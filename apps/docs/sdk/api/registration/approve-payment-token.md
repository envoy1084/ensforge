...
const result = await ens.registration.approvePaymentToken({
...
const program = ens.registration.approvePaymentToken.effect(parameters);
...
```ts
const call = ens.registration.approvePaymentToken.call(parameters);
```

...
const result = await ens.ownership.setManager({
  name: "example.eth",
...
const program = ens.ownership.setManager.effect(parameters);
...
```ts
const call = ens.ownership.setManager.call(parameters);
```

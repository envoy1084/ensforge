...
const result = await ens.ownership.transferRegistrant({
  name: "example.eth",
  to: "value",
});
```
...
```ts
import type { TransferRegistrantParameters } from "@ensforge/sdk";
```
...
const program = ens.ownership.transferRegistrant.effect(parameters);
...
```ts
const call = ens.ownership.transferRegistrant.call(parameters);
```

## Error

```ts
import type { TransferRegistrantError } from "@ensforge/sdk";
```

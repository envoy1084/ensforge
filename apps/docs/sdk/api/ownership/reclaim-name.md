...
const result = await ens.ownership.reclaimName({
  name: "example.eth",
...
const program = ens.ownership.reclaimName.effect(parameters);
...
```ts
const call = ens.ownership.reclaimName.call(parameters);
```

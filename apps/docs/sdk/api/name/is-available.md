...
const result = await ens.name.isAvailable({
  name: "example.eth",
});
```
...
const program = ens.name.isAvailable.effect(parameters);
...
```ts
const request = ens.name.isAvailable.request(parameters);
```

## Error

```ts
import type { IsAvailableError } from "@ensforge/sdk";
```

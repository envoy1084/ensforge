...
const result = await ens.name.getNameStatus({
  name: "example.eth",
});
```
...
const program = ens.name.getNameStatus.effect(parameters);
...
```ts
const request = ens.name.getNameStatus.request(parameters);
```

...
```ts
type IsWrappedParameters = Parameters<typeof sdk.name.isWrapped>[0];
```
...
```ts
type IsWrappedResult = Awaited<ReturnType<typeof sdk.name.isWrapped>>;
```
...
```ts
const effect = sdk.name.isWrapped.effect(parameters);

...
```ts
type GetTokenIdParameters = Parameters<typeof sdk.name.getTokenId>[0];
```
...
```ts
type GetTokenIdResult = Awaited<ReturnType<typeof sdk.name.getTokenId>>;
```
...
```ts
const effect = sdk.name.getTokenId.effect(parameters);

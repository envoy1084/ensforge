...
const result = await ens.name.isMigrated({
  name: "example.eth",
});
```
...
const program = ens.name.isMigrated.effect(parameters);
...
```ts
const request = ens.name.isMigrated.request(parameters);
```

## Error

```ts
import type { IsMigratedError } from "@ensforge/sdk";
```

...
const result = await ens.name.getRegistrant({
  name: "example.eth",
});
```
...
const program = ens.name.getRegistrant.effect(parameters);
...
```ts
const request = ens.name.getRegistrant.request(parameters);
```

## Error

```ts
import type { GetRegistrantError } from "@ensforge/sdk";
```

...
const result = await ens.resolution.getResolver({
  name: "example.eth",
});
```
...
const program = ens.resolution.getResolver.effect(parameters);
...
```ts
const request = ens.resolution.getResolver.request(parameters);
```

...
const result = await ens.resolution.resolveBatch({
  calls: [],
});
```
...
const program = ens.resolution.resolveBatch.effect(parameters);
...
```ts
const request = ens.resolution.resolveBatch.request(parameters);
```

...
const result = await ens.resolution.resolve({
  name: "example.eth",
  data: "0x",
});
```
...
const program = ens.resolution.resolve.effect(parameters);
...
```ts
const request = ens.resolution.resolve.request(parameters);
```

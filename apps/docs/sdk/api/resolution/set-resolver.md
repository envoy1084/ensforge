...
const result = await ens.resolution.setResolver({
  name: "example.eth",
...
```ts
import type { SetResolverParameters } from "@ensforge/sdk";
```
...
const program = ens.resolution.setResolver.effect(parameters);
...
```ts
const call = ens.resolution.setResolver.call(parameters);
```

## Error

```ts
import type { SetResolverError } from "@ensforge/sdk";
```
